...
import { THEME, TRANSACTION_ACTIONS } from '../constants/equipmentConstants';
  ...
  return (
    <>
      <Header>
        {(toggleDrawer) => (
          <>
            <TopBar title="Transaction History" onMenuClick={toggleDrawer} subtitle="LIBRARY EQUIPMENT ENCODING" />
      ...
      <Box sx={{ p: 3, maxWidth: 1300, margin: '0 auto' }}>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
          <Typography sx={{ fontFamily: font, fontWeight: 700, fontSize: 20, color: THEME.navy }}>
            Inventory Transaction History
          </Typography>
          <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
            <TextField
              size="small" placeholder="Search asset, user, destination"
            ...
            <TextField
              ...
              sx={{ minWidth: 180 }}
            >
              <MenuItem value="All" sx={{ fontFamily: font }}>All Actions</MenuItem>
              {Object.values(TRANSACTION_ACTIONS).map((a) => (
                <MenuItem key={a} value={a} sx={{ fontFamily: font }}>{a}</MenuItem>
              ))}
            </TextField>
          </Box>
        </Box>
            ...
            <Table size="small">
              <TableHead>
                <TableRow sx={{ backgroundColor: '#fafafa' }}>
                  {['Date', 'Asset', 'Action', 'Qty Change', 'Previous Qty', 'New Qty', 'Destination', 'User'].map((h) => (
                ...
                {paged.map((t) => {
                  ...
                  return (
                    <TableRow key={t.transaction_id} sx={{ '&:hover': { backgroundColor: '#fafafa' } }}>
                      ...
                      <TableCell sx={{ fontFamily: font, fontSize: 13, fontWeight: 600 }}>{t.asset_name}</TableCell>
                      <TableCell>
