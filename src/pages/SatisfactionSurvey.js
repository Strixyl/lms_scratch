...
const SatisfactionSurvey = () => {
  ...
  // Define course options for each college
    switch (college) {
      case 'CARES': // College of Agriculture, Resources & Environmental Sciences
        setAvailableCourses([
          'Agriculture',
      ...
      setAvailableCourses([
        ...
        'Biology with specialization in Medical Biology',
      ...
      case 'CCS':
        ...
        setAvailableCourses([
          ...
          'Secondary Education major in Special Needs Education',
        ]);
        break;
    ...
    return (
        <Header>
          ...
          <Box sx={{
            ...
            <Grid container
                  spacing={0}  // Remove gutter spacing between Grid items
                  sx={{
                    height: '100%',
                    width: '100%',
                    margin: 0,
      ...
      <Grid item xs={12} md={3} sx={{ bgcolor: '#000d3a', color: 'white', p: { xs: 2, sm: 3, md: 4 }, height: '100%', boxSizing: 'border-box', overflowY: 'auto', width:'30%'}}>
        ...
        <FormControl fullWidth sx={{ mt: 3 }}>
          ...
          <Select label="College" value={selectedCollege}
        ...
        <FormControl fullWidth sx={{ mt: 3 }}>
          ...
          <Select label="Course" value={selectedCourse}
                              onChange={(e) => setSelectedCourse(e.target.value)} // ✅ This tracks selection
                ...
                {ratingOptions.map((option) => (
                  <Box
                    ...
                    sx={{
                      textAlign: 'center',
                      cursor: 'pointer',
                      filter: responses[index] === option.id ? 'none' : 'grayscale(100%)',
                      transition: 'filter 0.2s',
                    }}
                  >
                  <img
                  src={
                    responses[index] === option.id && animating[index] === option.id
                      ? option.animated // show animation only if selected AND animating
                      : option.static   // always fallback to static for everything else
                  }
                  alt={option.label}
                  width="40"
                  height="40"
                ...
                {ratingOptions.map((option) => (
                  <Box
                    ...
                    onClick={() => {
                      ...
                      {/*setTimeout(() => {
                        const resetAnimating = [...animating];
                        resetAnimating[index] = null;
                    ...
                    sx={{
                      textAlign: 'center',
                      cursor: 'pointer',
                      filter: responses[index + 5] === option.id ? 'none' : 'grayscale(100%)',
                      transition: 'filter 0.2s',
                    }}
                  >
                  <img
                  src={
                    responses[index + 5 ] === option.id && animating[index + 5] === option.id
                      ? option.animated // show animation only if selected AND animating
                      : option.static   // always fallback to static for everything else
                  }
                  alt={option.label}
          ...
          />
            ...
            >
              <Alert
                ...
                sx={{
                  width: '100%',
                  fontSize: '1.1rem',         // ⬅️ Increase font size
                  padding: '16px 24px',       // ⬅️ Add padding inside the alert
                  minWidth: '300px',          // ⬅️ Optional: Minimum width
                  fontFamily: 'Poppins, sans-serif',
                }}
              >
