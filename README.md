...
1. **Machine Learning Pipeline & Model Retraining (`backend/ml/`)**:
   ...
   - **Mixed-Sentiment & Dual-Topic Clause Handling**: Upgraded `sentiment_service.py` (`POST /analyze` & `POST /categorize`) and `naive_bayes.py` (`predict_with_fallback()`) with clause-aware resolution (`get_clause_category()`). Multi-clause comments are split on contrast conjunctions (`although`, `however`, `but`, `though`), applying most-negative-wins sentiment aggregation and dynamically binding the overall Category to the **winning negative complaint clause** (e.g. Wi-Fi dropping $\rightarrow$ `Facilities`). If feature dilution causes full-text prediction to output `Other/Uncategorized`, the system overrides using explicit domain keywords (`aircon`, `wifi`, `librarian`, `textbook`) and evaluates per-clause categories.
