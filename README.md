...
1. **Machine Learning Pipeline & Model Retraining (`backend/ml/`)**:
   ...
   - **Mixed-Sentiment & Dual-Topic Clause Handling**: Upgraded `sentiment_service.py` (`POST /analyze` & `POST /categorize`) with clause-aware resolution (`get_clause_category()`). Multi-clause comments are split on contrast conjunctions (`although`, `however`, `but`), applying most-negative-wins sentiment aggregation and dynamically binding the overall Category to the **winning negative complaint clause** (e.g. Wi-Fi dropping $\rightarrow$ `Facilities`), preventing misattribution to praised entities (e.g. Staff).
